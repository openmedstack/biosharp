...
public class FastQReader
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogInformation("Read {File} to {Position}/{Length}", file.Name, file.Position, file.Length);
            ...
            var q = await reader.ReadLineAsync(cancellationToken).ConfigureAwait(false);
            if (q == "+") q = await reader.ReadLineAsync(cancellationToken).ConfigureAwait(false);
