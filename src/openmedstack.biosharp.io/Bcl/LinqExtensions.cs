namespace OpenMedStack.BioSharp.Io.Bcl
{
    ...
    {
        private class Group<TKey, TElement> : IGrouping<TKey, TElement>, IDisposable
        {
            private readonly BlockingCollection<TElement> _items;

            public Group(TKey key, BlockingCollection<TElement> items)
            {
                _items = items;
                Key = key;
            }
            ...
            {
                return _items.GetConsumingEnumerable(CancellationToken.None).GetEnumerator();
            }
        ...
        public static async IAsyncEnumerable<IGrouping<TKey, T>> GroupInto<T, TKey>(
            this IAsyncEnumerable<T> source,
            Func<T, TKey> keySelector,
            [EnumeratorCancellation] CancellationToken cancellationToken = default)
        where TKey : IEquatable<TKey>
        {
            Dictionary<TKey, BlockingCollection<T>> memory = new();
            await foreach (var item in source.ConfigureAwait(false))
            {
                ...
                if (!memory.TryGetValue(key, out var sequence))
                {
                    sequence = new BlockingCollection<T>();
                    memory.Add(key, sequence);
                    yield return new Group<TKey, T>(key, sequence);
                }

                sequence.Add(item, cancellationToken);
            }

            foreach (var collection in memory)
            {
