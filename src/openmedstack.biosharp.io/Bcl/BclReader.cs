...
public partial class BclReader : IAsyncDisposable, IAsyncEnumerable<ReadData[]>
{
    ...
    static BclReader()
    {
        ...
        // For all other byte values, return the raw quality (byte >> 2) so that the
        // quality evaluation strategy can correctly log and revise low-quality reads.
        // The clamping to minimum quality happens in BclQualityEvaluationStrategy.
        foreach (var x in Enumerable.Range(1, 255))
        {
            var i = (byte)x;
            BclBaseLookup[i] = BaseLookup[i & BaseMask];
            BclQualLookup[i] = (char)(i >> 2); // raw quality, NOT clamped
        }
    }
    ...
    {
        var reader = new BclReader(
            [new Read { IsIndexedRead = "N", NumCycles = 1, Number = 1, Type = ReadType.T }],
            tileIndexRecord,
            bclQualityEvaluationStrategy,
            applyEamss,
            queueSize,
        ...
        // Validate file size matches expected size (header + clusters * cycles)
        var fileSize = bclFileInfo.Length;
        var expectedSize = HeaderSize + reader.NumClustersPerCycle[0] * 1; // 1 cycle for single-read BCL
    ...
    public int Tile
    {
        get { return _tileIndexRecord.Tile; }
    }
        ...
        while (readIndex < _tileIndexRecord.NumClustersInTile && !cancellationToken.IsCancellationRequested)
        {
            ...
            for (var read = 0; read < NumReads; ++read)
            {
                ...
                for (var cycle = firstCycle; cycle < readLen; ++cycle)
                {
                    ...
                    await ReadAndUpdateData(clustersRead, bclDatas, read, cycle, totalCycleCount, cancellationToken)
    ...
    private void UpdateClusterBclDatas(Span<BclData> bclDatas, int read, int cycle, Span<byte> buffer)
    {
        ...
        for (var dataIdx = 0; dataIdx < numClusters; ++dataIdx)
        {
            ...
            data.Qualities[read][cycle] =
                _bclQualityEvaluationStrategy.ReviseAndConditionallyLogQuality(BclQualLookup[b]);
        }
    }
