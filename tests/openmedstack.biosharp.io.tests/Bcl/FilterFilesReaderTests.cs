...
public class FilterFilesReaderTests : IAsyncLifetime
{
    ...
    public async ValueTask InitializeAsync()
    {
        ...
        if (dataDir == null) Assert.Skip("Cannot locate executing assembly directory");

        var root = dataDir.Parent?.Parent?.Parent?.Parent;
        if (root == null) Assert.Skip("Cannot navigate to data directory");

        var filterPath = Path.Combine(root.FullName, "data/sampledata/Data/Intensities/BaseCalls/L001/s_1_1101.filter");

        if (!File.Exists(filterPath)) Assert.Skip($"No .filter test file available at: {filterPath}");
